...
TIM_HandleTypeDef *ESfanTimList[6] = {&htim4, &htim2, &htim5, &htim3,
                                      &htim1, &htim8}; // PB6 PA15 PA0 PB1 PA8  PC8
//GPIO_TypeDef VibrationPort[5] = {Vibration0_GPIO_Port,Vibration1_GPIO_Port,Vibration2_GPIO_Port,
 //                             Vibration3_GPIO_Port,Vibration4_GPIO_Port};
uint32_t VibrationPin[5] = {VB1_Pin,VB2_Pin,VB3_Pin,VB4_Pin,VB5_Pin};
...
//    ES.fan[5].mode = 4; // 1
  ...
  if (ES.state == 6)
  {
    ...
    for(uint8_t i = 0; i < 5; i++)
    {
    disturbance += (disturbance_flag[i]>0);
    }
    if(disturbance >= 1)
    {
//    fan_list_init();
...
//    boot_init();
    ...
    for(uint8_t i=0; i<=4;i++)
			{    
      //HAL_GPIO_WritePin(VibrationPort[i] , VibrationPin[i],GPIO_PIN_RESET);
    disturbance_init();
    }  

  if (ES.timeStamp - ES.last_mode_timeStamp > 1700/2 * fan_change_time|| ES.status_same)//3149
  {
    if (ES.status_same)
    ES.status_same = 0;
    ES.state = 0;
  ...
  for (uint8_t i = 0; i < 6; i++)
  {
    ...
    case 0:
      close_all();
			if(i==4)
      HAL_GPIO_WritePin(VB5_GPIO_Port,VibrationPin[i],GPIO_PIN_RESET);
			else 
			HAL_GPIO_WritePin(GPIOA ,VibrationPin[i],GPIO_PIN_RESET);
      break;
    ...
    case 2:
      ...
      LED_YUAN_Init(RGB);
			Vibra_monitor();
			if(i==4)
      HAL_GPIO_WritePin(VB5_GPIO_Port,VibrationPin[i],GPIO_PIN_SET);
			else 
			HAL_GPIO_WritePin(GPIOA ,VibrationPin[i],GPIO_PIN_SET);
      break;
    }
...
void ctr_fan1(uint8_t s, int RGB,int fan_change_time)
{
  ...
  if (ES.timeStamp - ES.last_mode_timeStamp > 3149/2 * 1000 || ES.status_same)
  {
    if (ES.status_same)
      ES.status_same = 0;
    ES.state = 0;
...
void fan_logic(uint8_t s)
{
  ...
  uint8_t rand_num_use = 0, use_num = ES.timeStamp % 5 + 1; //
  //
   while (use_num--)
...
void Test2(int s, int RGB)
{
  ...
  light_flow(s, RGB, flag_flow); 
  flag_flow = (flag_flow == 2) ? 0 : flag_flow + 1;
  LED_YUAN_Init(RGB);

  HAL_TIM_PWM_Start_DMA(ESfanTimList[5], ESfanChannelList[5], &WS2812B_Bit[0],
                          24 * All_Number + 1);
}
...
void Test3(int s, int RGB)//全亮
{
      ...
      flag_flow = (flag_flow == 2) ? 0 : flag_flow + 1;
      LED_YUAN_Init(RGB);
			HAL_GPIO_WritePin(GPIOB ,VibrationPin[2],GPIO_PIN_SET);
			HAL_TIM_PWM_Start_DMA(ESfanTimList[2], ESfanChannelList[2], &WS2812B_Bit[0],
                          24 * All_Number + 1);
			HAL_GPIO_WritePin(Vibration0_GPIO_Port ,VibrationPin[0],GPIO_PIN_RESET);
			HAL_GPIO_WritePin(GPIOB ,VibrationPin[3],GPIO_PIN_RESET);
			HAL_GPIO_WritePin(GPIOB ,VibrationPin[4],GPIO_PIN_RESET);
			HAL_GPIO_WritePin(GPIOB ,VibrationPin[1],GPIO_PIN_RESET);
			HAL_Delay(100);
}
